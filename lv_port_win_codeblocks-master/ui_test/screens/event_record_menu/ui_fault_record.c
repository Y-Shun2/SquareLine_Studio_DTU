...
lv_group_t * ui_fault_record_group = NULL;
lv_obj_t * ui_fault_record_title = NULL;
lv_obj_t * ui_fault_record_title_label = NULL;
...
// event funtions
void ui_fault_record_event(lv_event_t * e)
{
...
// build funtions
void ui_fault_record_screen_init(void)
{
    ui_fault_record_title = lv_obj_create(NULL);
    lv_obj_clear_flag(ui_fault_record_title, LV_OBJ_FLAG_SCROLLABLE);      /// Flags

    ui_fault_record_group = lv_group_create(); // 创建一个新的对象组
    lv_indev_t *indev = lv_win32_keypad_device_object;

    ui_fault_record_title_label = lv_label_create(ui_fault_record_title);
    lv_obj_set_width(ui_fault_record_title_label, lv_pct(100));
    lv_obj_set_height(ui_fault_record_title_label, LV_SIZE_CONTENT);    /// 16
    lv_label_set_text(ui_fault_record_title_label, "Fault Record");
    lv_obj_set_align(ui_fault_record_title_label, LV_ALIGN_TOP_MID);
    lv_obj_add_style(ui_fault_record_title_label, &style_title, 0);
...
void ui_fault_record_screen_destroy(void)
{
    if(ui_fault_record_title) lv_obj_del(ui_fault_record_title);

    // NULL screen variables
    ui_fault_record_title = NULL;
    ui_fault_record_title_label = NULL;
}
