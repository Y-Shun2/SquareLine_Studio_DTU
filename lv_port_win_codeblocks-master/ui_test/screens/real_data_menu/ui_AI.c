...
lv_group_t * ui_AI_group = NULL;
lv_obj_t * ui_AI_title = NULL;
lv_obj_t * ui_AI_title_label = NULL;
lv_obj_t * ui_point_AI_button = NULL;
lv_obj_t * ui_point_AI_label = NULL;
lv_obj_t * ui_all_AI_button = NULL;
lv_obj_t * ui_all_AI_label = NULL;
...
void ui_AI_event(lv_event_t * e)
{
    ...
    switch(event_code)
    {
        ...
        case LV_EVENT_KEY:
            ...
            {
                case LV_KEY_ENTER:
                    if (btn == ui_point_AI_button)
                    {
                        lv_indev_set_group(indev, ui_point_ai_group);
                        _ui_screen_change(&ui_point_ai_title, LV_SCR_LOAD_ANIM_FADE_ON, 500, 0, &ui_point_ai_screen_init);
                    }
                    if (btn == ui_all_AI_button)
                    {
                        lv_indev_set_group(indev, ui_all_ai_group);
                        _ui_screen_change(&ui_all_ai_title, LV_SCR_LOAD_ANIM_FADE_ON, 500, 0, &ui_all_ai_screen_init);
                    }
                    break;
...
{
    ui_AI_title = lv_obj_create(NULL);
    lv_obj_clear_flag(ui_AI_title, LV_OBJ_FLAG_SCROLLABLE);      /// Flags

    ui_AI_group = lv_group_create(); // 创建一个新的对象组
    lv_indev_t *indev = lv_win32_keypad_device_object;

    lv_obj_set_style_outline_width(ui_AI_title, 0, LV_PART_MAIN | LV_STATE_FOCUSED);//关闭所有焦点轮廓
    ...
    /*-----创建点表遥测菜单项-----*/
    ui_point_AI_button = lv_btn_create(ui_AI_title);
    ...
    /*-----创建全遥测菜单项-----*/
    ui_all_AI_button = lv_btn_create(ui_AI_title);
    ...
    lv_group_add_obj(ui_AI_group, ui_point_AI_button);
    lv_group_add_obj(ui_AI_group, ui_all_AI_button);

    lv_indev_set_group(indev, ui_AI_group); // 将输入设备与对象组关联

    lv_group_focus_obj(ui_point_AI_button);
}

void ui_AI_screen_destroy(void)
{
    if(ui_AI_title) lv_obj_del(ui_AI_title);

    // NULL screen variables
    ui_AI_title = NULL;
    ui_AI_title_label = NULL;
    ui_point_AI_button = NULL;
    ui_point_AI_label = NULL;
    ui_all_AI_button = NULL;
    ui_all_AI_label = NULL;
}
