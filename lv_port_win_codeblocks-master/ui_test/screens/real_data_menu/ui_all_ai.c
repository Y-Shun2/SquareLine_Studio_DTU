...
lv_group_t * ui_all_ai_group = NULL;
lv_obj_t * ui_all_ai_title = NULL;
lv_obj_t * ui_all_ai_title_label = NULL;
...
// event funtions
void ui_all_ai_event(lv_event_t * e)
{
...
// build funtions
void ui_all_ai_screen_init(void)
{
    ui_all_ai_title = lv_obj_create(NULL);
    lv_obj_clear_flag(ui_all_ai_title, LV_OBJ_FLAG_SCROLLABLE);      /// Flags

    ui_all_ai_group = lv_group_create(); // 创建一个新的对象组
    lv_indev_t *indev = lv_win32_keypad_device_object;
...
void ui_all_ai_screen_destroy(void)
{
    if(ui_all_ai_title) lv_obj_del(ui_all_ai_title);

    // NULL screen variables
    ui_all_ai_title = NULL;
    ui_all_ai_title_label = NULL;
}
