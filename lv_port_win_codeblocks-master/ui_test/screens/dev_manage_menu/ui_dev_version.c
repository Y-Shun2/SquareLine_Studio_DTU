...
lv_group_t * ui_dev_version_group = NULL;
...
// event funtions
void ui_dev_version_event(lv_event_t * e)
{
...
// build funtions
void ui_dev_version_screen_init(void)
{
    ui_dev_version_title = lv_obj_create(NULL);
    lv_obj_clear_flag(ui_dev_version_title, LV_OBJ_FLAG_SCROLLABLE);      /// Flags

    ui_dev_version_group = lv_group_create(); // 创建一个新的对象组
    lv_indev_t *indev = lv_win32_keypad_device_object;
...
{
    if(ui_dev_version_title) lv_obj_del(ui_dev_version_title);
