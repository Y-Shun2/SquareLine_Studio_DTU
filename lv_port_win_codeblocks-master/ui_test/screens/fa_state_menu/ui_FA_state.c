...
lv_group_t * ui_FA_state_group = NULL;
lv_obj_t * ui_FA_state_title = NULL;
lv_obj_t * ui_FA_state_title_label = NULL;
...
// event funtions
void ui_FA_state_event(lv_event_t * e)
{
...
// build funtions
void ui_FA_state_screen_init(void)
{
    ui_FA_state_title = lv_obj_create(NULL);
    lv_obj_clear_flag(ui_FA_state_title, LV_OBJ_FLAG_SCROLLABLE);      /// Flags

    ui_FA_state_group = lv_group_create(); // 创建一个新的对象组
    lv_indev_t *indev = lv_win32_keypad_device_object;

    ui_FA_state_title_label = lv_label_create(ui_FA_state_title);
    lv_obj_set_width(ui_FA_state_title_label, lv_pct(100));
    lv_obj_set_height(ui_FA_state_title_label, LV_SIZE_CONTENT);    /// 16
    lv_label_set_text(ui_FA_state_title_label, "FA State");
    lv_obj_set_align(ui_FA_state_title_label, LV_ALIGN_TOP_MID);
    lv_obj_add_style(ui_FA_state_title_label, &style_title, 0);
...
void ui_FA_state_screen_destroy(void)
{
    if(ui_FA_state_title) lv_obj_del(ui_FA_state_title);

    // NULL screen variables
    ui_FA_state_title = NULL;
    ui_FA_state_title_label = NULL;
}
