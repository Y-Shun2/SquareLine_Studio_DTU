...
lv_group_t * ui_sys_maint_group = NULL;
lv_obj_t * ui_sys_maint_title = NULL;
lv_obj_t * ui_sys_maint_title_label = NULL;
...
// event funtions
void ui_sys_maint_event(lv_event_t * e)
{
...
// build funtions
void ui_sys_maint_screen_init(void)
{
    ui_sys_maint_title = lv_obj_create(NULL);
    lv_obj_clear_flag(ui_sys_maint_title, LV_OBJ_FLAG_SCROLLABLE);      /// Flags

    ui_sys_maint_group = lv_group_create(); // 创建一个新的对象组
    lv_indev_t *indev = lv_win32_keypad_device_object;

    lv_obj_set_style_outline_width(ui_sys_maint_title, 0, LV_PART_MAIN | LV_STATE_FOCUSED);//关闭所有焦点轮廓
...
void ui_sys_maint_screen_destroy(void)
{
    if(ui_sys_maint_title) lv_obj_del(ui_sys_maint_title);

    // NULL screen variables
    ui_sys_maint_title = NULL;
    ui_sys_maint_title_label = NULL;
}
