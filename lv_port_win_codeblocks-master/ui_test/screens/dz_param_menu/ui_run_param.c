...
lv_group_t * ui_run_param_group = NULL;
lv_obj_t * ui_run_param_title = NULL;
lv_obj_t * ui_run_param_title_label = NULL;
lv_obj_t * ui_comrun_param_button = NULL;
lv_obj_t * ui_comrun_param_label = NULL;
lv_obj_t * ui_jgrun_param_button = NULL;
lv_obj_t * ui_jgrun_param_label = NULL;
...
// event funtions
void ui_run_param_event(lv_event_t * e)
{
    ...
    switch(event_code)
    {
        ...
        case LV_EVENT_KEY:
            ...
            {
                case LV_KEY_ENTER:
                    if (btn == ui_comrun_param_button)
                    {
                        ui_comrun_param_screen_init();
                        lv_indev_set_group(indev, ui_comrun_param_group);
                        _ui_screen_change(&ui_comrun_param_title, LV_SCR_LOAD_ANIM_FADE_ON, 500, 0, &ui_comrun_param_screen_init);
                    }
                    if (btn == ui_jgrun_param_button)
                    {
                        ui_jgrun_param_screen_init();
                        lv_indev_set_group(indev, ui_jgrun_param_group);
                        _ui_screen_change(&ui_jgrun_param_title, LV_SCR_LOAD_ANIM_FADE_ON, 500, 0, &ui_jgrun_param_screen_init);
                    }
                    break;
...
// build funtions
void ui_run_param_screen_init(void)
{
    ui_run_param_title = lv_obj_create(NULL);
    lv_obj_clear_flag(ui_run_param_title, LV_OBJ_FLAG_SCROLLABLE);      /// Flags

    ui_run_param_group = lv_group_create(); // 创建一个新的对象组
    lv_indev_t *indev = lv_win32_keypad_device_object;
    ...
    /*-----创建公共运行参数菜单项-----*/
    ui_comrun_param_button = lv_btn_create(ui_run_param_title);
    ...
    /*-----创建间隔运行参数菜单项-----*/
    ui_jgrun_param_button = lv_btn_create(ui_run_param_title);
    ...
    lv_group_add_obj(ui_run_param_group, ui_comrun_param_button);
    lv_group_add_obj(ui_run_param_group, ui_jgrun_param_button);

    lv_indev_set_group(indev, ui_run_param_group); // 将输入设备与对象组关联

    lv_group_focus_obj(ui_comrun_param_button);
}

void ui_run_param_screen_destroy(void)
{
    if(ui_run_param_title) lv_obj_del(ui_run_param_title);

    // NULL screen variables
    ui_run_param_title = NULL;
    ui_run_param_title_label = NULL;
    ui_comrun_param_button = NULL;
    ui_comrun_param_label = NULL;
    ui_jgrun_param_button = NULL;
    ui_jgrun_param_label = NULL;
}
